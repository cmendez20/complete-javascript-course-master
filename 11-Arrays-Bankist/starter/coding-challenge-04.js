...
const findDog = function (ownerName) {
  const dogsOwner = dogs.filter(dog => dog.owners.includes(ownerName));
  // console.log(dogsOwner);

  return dogsOwner.curFood > dogsOwner.recommendedFood * 0.9
    ? `${ownerName}'s dog is eating too much`
    : `${ownerName}'s dog is eating too little`;
};
